...
function beans_enqueue_uikit_components() {
    ...
    // TODO REMOVE
//    $PATH = ABSPATH . 'wp-content/plugins/beans-uikit2/assets/';
    $assets_path = plugin_dir_url(__FILE__);
//    ddd(plugin_dir_url(__FILE__));

    // Enqueue UIkit overwrite theme folder.
    beans_uikit_enqueue_theme( 'beans', $assets_path . 'less/uikit-overwrite' );

    // Add the theme style as a UIkit fragment to have access to all the variables.
    beans_compiler_add_fragment( 'uikit', $assets_path . 'less/style.less', 'less' );

    // Add the Gutenberg Styles.
    beans_compiler_add_fragment( 'uikit', $assets_path . 'less/gutenberg/color-pallet.less', 'less' );
    beans_compiler_add_fragment( 'uikit', $assets_path . 'less/gutenberg/font-sizes.less', 'less' );
    ...
    if ( current_theme_supports( 'beans-default-styling' ) ) {
        beans_compiler_add_fragment( 'uikit', $assets_path . 'less/default.less', 'less' );
    }
}
